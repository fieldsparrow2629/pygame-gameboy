...
def draw_alien(a):
    x = a[0]
    y = a[1]
    pygame.draw.rect(screen,WHITE,a[:4])
...
for i in range(10):
    ...
    speed_x = random.randrange(-1,1,2)
    speed_y = random.randrange(-1,1,2)
    a = [x,y,20,20,speed_x,speed_y]
    aliens.append(a)
...
while not done:
    ...
    for a in aliens:
        ...
        if a[0] + a[4] > 510 or a[0] + a[4] < 305:
            a[4] = -1 * a[4]
        if a[1] + a[5] > 160 or a[1] + a[5] < 80:
            a[5] = -1 * a[5]
    ...
    if show_instruction:
        ...
        inst4 = SMALL_FONT.render('Press "m" to enter Musk Mode.',True,WHITE)
        inst5 = BIG_FONT.render('Press ENTER to begin.',True, WHITE)
        ...
        screen.blit(inst4, [width//2 - width4//2,360])
        screen.blit(inst5, [width//2 - width5//2,400])
